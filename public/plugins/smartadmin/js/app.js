...
function App(){
	...
	$('.plus-global-action').unbind('click').on('click', function () {
		if ($(this).hasClass('open')) {
			$(this).removeClass('open');
			$('.plus-global-action-list').removeClass('open');
			$(this).find('i').removeClass('fa-times').addClass('fa-plus');
		}
		else {
			$(this).addClass('open');
			$('.plus-global-action-list').addClass('open');
			$(this).find('i').removeClass('fa-plus').addClass('fa-times');
		}
	});

	$(document).on('click', function(e) {
		if (!$(e.target).hasClass('plus-global-action') && $(e.target).closest('.plus-global-action').length == 0 && $(e.target).closest('.plus-global-action-list').length == 0) {
			e.stopPropagation();

			if ($('.plus-global-action').hasClass('open')) {
				$('.plus-global-action').removeClass('open');
				$('.plus-global-action-list').removeClass('open');
				$('.plus-global-action').find('i').removeClass('fa-times').addClass('fa-plus');
			}
		}
	});
...
App.prototype.bindApp = function () {
	$('.header-search').unbind('submit');
	/*
	 * Select2
	 */
	if ($('select.select-single').length > 0) 
		$('select.select-single').select2().on("change", function (e) { });
	
	if ($('select.select-multiple').length > 0)
		$('select.select-multiple').select2().on("change", function (e) { });
...
$(window).load(function() {
	$(".stuff-to-preload").css("display", "none");
});
...
$(document).ready(function() {
	...
	$('#txt-change-password-new').keyup(function() {
		$('#result').html(checkPassStrength($('#txt-change-password-new').val()))
	});
	...
	var $change_password_validator = $("#frm-change-password").validate({
		rules: {
			'password': { required: true },
			'confirm_password': { required: true, equalTo: "#txt-change-password-new" }
		},
		messages: {
	...
	$('#frm-change-password').unbind('submit').on('submit', function (event)
	{		
		event.preventDefault();

		var $valid = $("#frm-change-password").valid();

    	if (!$valid) {
		    $change_password_validator.focusInvalid();
		    return false;
		}

		var password = $('#txt-change-password-new').val();
		...
		modal.show('info', 'fa-refresh fa-spin', 'Saving...', 'Please wait while we are saving your new password.', null);

		$.ajax({
			url: '/auth/ajaxChangePassword',
			type: 'POST',
			dataType: "json",
			data: data,
			success: function(data){
				if (data.status == 'OK')
				modal.set('success', 'fa-check-circle', 'Success', 'New password saved successfully.', { ok : function () {
				...
				modal.set('danger', 'fa-times-circle', 'Oops', xhr.responseText, { ok : true });
			}
		});
	});
	...
	$('#frm-upload-photo').unbind('submit').on('submit', function (event) {
        event.preventDefault();
        $('#btn-upload-photo-logo').trigger('click');
    });

    $('#btn-upload-photo').unbind('click').on('click', function (event) {
        uploadCrop.result('canvas', 'viewport').then(function (src) {
           var id = $('#hdn-upload-photo-id').val();
           ...
           var CSRF = $("meta[name='csrf-token']").attr('content');
            ...
            request.open('POST', '/persons/' + id + '/ajaxUploadImage');
	...
	$('#logout a').click(function(e) {
		...
		$.SmartMessageBox({
			...
			content : $.logoutMSG || "Thank you for using ChrisList!",
			buttons : '[No][Yes]'
...
if($.navAsAjax)
{
	$('.page-preloader').height($(window).height())

    // fire links with targets on different window
    $(document).on('click', '#left-panel nav a[target="_blank"]', function(e) {
	    e.preventDefault();
    ...
    $(document).on('click', '#left-panel nav a[target="_top"]', function(e) {
	    e.preventDefault();
	    var $this = $(e.currentTarget);
    ...
    $(document).on('click', '#left-panel nav a[href="#"]', function(e) {
	    e.preventDefault();
    });
}
...
function checkURL() {
	...
	if (url) {
		...
		var title = ($('#left-panel nav a[href="' + url + '"]').attr('title'))
		...
		var $this = $('#left-panel nav > ul > li:first-child > a[href!="#"]');
...
function loadURL(url, container) {
	util.partialRedirect(url);
}

// UPDATE BREADCRUMB
function drawBreadCrumb() {
	var nav_elems = $('#left-panel nav li.active > a'), count = nav_elems.length;
